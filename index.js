...
function sumNumbers(array) {
	...
	for (let i = 0; i < array.length; i++) {
		if(_.isNumber(array[i])){
			realNumbers.push (array[i])
		}	
	}

	return _.sum(realNumbers);
...
function groupClassByInstructor(collection) {
	// return _.sortBy(grouped, ['instructor'])
...
function omitAgeFromMembers(collection) {
	let newArr = [];
	collection.forEach((singleClass) => {
		newArr.push(_.omit(singleClass, "age"))
	});
	return newArr
};
...
function countClassesByInstructor(collection, instructor) {
	// const count = _.uniqBy(collection, 'instructor')
	// return count.length + 1
	const count = _.countBy(collection, (oneClass) => instructor === oneClass.instructor)

	return count.true || "There is no instructor by that name."
};
...
function getUniqueClasses(collection) {
	return _.uniqBy(collection, 'title').map((item)=> ({
		title: item.title,
...
function orderClassesByTitleAndLevel(collection) {
	...
	// console.log(sorted)
	const mapped = _.map(sorted, ({title, level, instructor}) => ({title, level, instructor}))
	// console.log(mapped)
	return mapped

	// return ((_.orderBy(collection, ['title', 'level'], ['asc','desc']))_.map(sorted, ({title, level, instructor}) => ({title, level, instructor})))
	// // console.log(mapped)
	// return mapped
