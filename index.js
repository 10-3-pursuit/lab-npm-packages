...
function numberOfKeys(obj) {
  // Initialize var to hold total keys, start at 0
  let total = 0
  _.forIn(obj, (key) => {
    total ++
  }) 
...
function sumNumbers(array) {
  return _.compact(array).reduce((acc, current)=> acc + current, 0)
}
...
function omitAgeFromMembers(collection) {
  let array = []
  collection.forEach((agelessMember) => {
    array.push(_.omit(agelessMember, "age"))
  })
  return array
}
/**
...
function countClassesByInstructor(collection, instructor) {
  const count = _.countBy(collection, (classes) => instructor === classes.instructor)
...
function getUniqueClasses(collection) {
  return _.uniqBy(collection, "title").map((singleClass) => ({
    priceInCents: singleClass.priceInCents,
...
function orderClassesByTitleAndLevel(collection){
  const inOrder = _.orderBy(collection, ['title', 'level'], [ , 'desc'])
  const finalCollection = _.map(inOrder, ({title, level, instructor}) => ({title, level, instructor}))
