...
function getUniqueClasses(collection) {
  let newObj = _.uniqBy(collection, "style")
  let newObj2 = newObj.map((singleClass) => _.pick(singleClass, ["title", "priceInCents"]))
  return newObj2
}
...
function orderClassesByTitleAndLevel(collection) {
  let newObj = collection.map((singleClass) => _.pick(singleClass, ["title", "instructor", "level"]))
  let objInOrder = _.orderBy(newObj, ["title", "level"], ["asc", "desc"])
  return objInOrder
}
