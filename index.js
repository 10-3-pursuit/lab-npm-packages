...
const membersWithoutAges = require("./data/members-without-ages.json");
...
function sumNumbers(array) {
  const numArray = _.compact(array);
...
function countClassesByInstructor(collection, instructorName) {
  ...
  const count = _.countBy(
    collection,
    (classes) => classes.instructor === instructorName
  );
...
function getUniqueClasses(collection) {
  return _.uniqBy(collection, "title").map((ele) => ({
    title: ele.title,
    priceInCents: ele.priceInCents,
  }));
}
...
function orderClassesByTitleAndLevel(collection) {
  // console.log(_.orderBy(collection, ["title", "level"], ["asc", "desc"]));
